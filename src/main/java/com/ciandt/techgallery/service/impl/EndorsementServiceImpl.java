...
public class EndorsementServiceImpl implements EndorsementService {
	...
	public Endorsement addOrUpdateEndorsement(EndorsementResponse endorsement, User user)
		...
		String googleId = userCanNotbeNull(user);
		// endorser user from techgallery datastore
		TechGalleryUser tgEndorserUser = getEndorserUser(googleId);
		// endorsed user from techgallery datastore
		TechGalleryUser tgEndorsedUser = getEndorsedUser(endorsement);
		// technology from techgallery datastore
		Technology technology = getTechnology(endorsement);
	...
	private Technology getTechnology(EndorsementResponse endorsement) throws BadRequestException {
	...
	public Endorsement addOrUpdateEndorsementPlusOne(EndorsementResponse endorsement, User user)
		...
		// saved for history purpose. if already exist one active endorsement,
		// set to inactive.
		// if not, add a new one as active
		final List<Endorsement> endorsements = endorsementDao.findActivesByUsers(tgEndorserUser, tgEndorsedUser,
				technology);
	...
	private TechGalleryUser getEndorsedUser(EndorsementResponse endorsement)
		...
		return tgEndorsedUser;
	}

	private TechGalleryUser getEndorserUser(String googleId) throws NotFoundException, BadRequestException {
		...
		return tgEndorserUser;
	}

	private String userCanNotbeNull(User user) throws OAuthRequestException {
		String googleId;
	...
	public Response getEndorsementsByTech(String techId, User user)
		...
		final List<EndorsementsGroupedByEndorsedTransient> grouped = groupEndorsementByEndorsed(endorsementsByTech,
				techId);
	...
	private void groupUsersWithSkill(final List<EndorsementsGroupedByEndorsedTransient> grouped,
		...
		for (Skill skill : techSkills) {
			...
			if (!isGrouped) {
				EndorsementsGroupedByEndorsedTransient newEndorsed = new EndorsementsGroupedByEndorsedTransient();
	...
	@Override
	public List<EndorsementsGroupedByEndorsedTransient> groupEndorsementByEndorsed(List<Endorsement> endorsements,
			String techId)
					throws BadRequestException, NotFoundException, InternalServerErrorException, OAuthRequestException {

		final Map<TechGalleryUser, List<TechGalleryUser>> mapUsersGrouped = new HashMap<TechGalleryUser, List<TechGalleryUser>>();
	...
	private List<EndorsementsGroupedByEndorsedTransient> transformGroupedUserMapIntoList(
			Map<TechGalleryUser, List<TechGalleryUser>> mapUsersGrouped, String techId)
					throws BadRequestException, NotFoundException, InternalServerErrorException, OAuthRequestException {
		final List<EndorsementsGroupedByEndorsedTransient> groupedList = new ArrayList<EndorsementsGroupedByEndorsedTransient>();

		for (final Map.Entry<TechGalleryUser, List<TechGalleryUser>> entry : mapUsersGrouped.entrySet()) {
	...
	@Override
	public Endorsement deleteEndorsement(EndorsementResponse endorsement, User user)
			throws InternalServerErrorException, BadRequestException, NotFoundException, OAuthRequestException {
		...
		List<Endorsement> endorsementList = endorsementDao.findActivesByUsers(tgEndorserUser, tgEndorsedUser,
				technology);
		Endorsement result = null;
		for (Endorsement endorsement2 : endorsementList) {
			result = endorsement2;
			endorsementDao.delete(endorsement2);
		}
		return result;
	}
}
