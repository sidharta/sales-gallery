...
public class TechnologyRecommendationCommentEndpoint {
	...
	private TechnologyRecommendationCommentService service = TechnologyRecommendationCommentServiceImpl.getInstance();

	/**
	 * Endpoint for adding a recomendation and comment.
	 * 
	 * @param recCommentTo
	 *            transient objetc for recomendationComment
	 * @param user
	 *            json with user informations
	 * @return recComment
	 ...
	 */
	@ApiMethod(name = "addRecommendationComment", path = "recommendation-comment", httpMethod = "post")
	public TechnologyRecommendation addRecommendationComment(TechnologyRecommendationCommentTO recCommentTo, User user)
			throws InternalServerErrorException, BadRequestException, NotFoundException {
		return service.addRecommendationComment(recCommentTo.getRecommendation(), recCommentTo.getComment(),
				recCommentTo.getTechnology(), user);
	}
	 ...
	 *             in case a request with problem were made.
	 */
	@ApiMethod(name = "deleteCommentAndRecommendation", path = "delete-recommendation-comment", httpMethod = "post")
	public void deleteCommentAndRecommendation(@Named("recommendId") Long recommendId,
			@Named("commentId") Long commentId, User user)
